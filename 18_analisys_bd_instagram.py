...
import os
import numpy as np
from datetime import datetime, timedelta # Importar datetime para el análisis mensual
...
def run_analysis(df, output_folder):
    ...
    # Se calcula el engagement diferenciando videos (por plays) del resto (por followers).
    ...
    # Agrupar para obtener seguidores y total de publicaciones
    # Usamos 'first' o 'max' para seguidores, asumiendo que es consistente por usuario en este df
    resumen_candidatos = df.groupby('username').agg(
        seguidores_actualizados=('followers_count', 'first'), 
        total_publicaciones=('post_id', 'count')
    ).reset_index()
    # Asegurarse de que no haya NaNs en seguidores si el 'first' falló
